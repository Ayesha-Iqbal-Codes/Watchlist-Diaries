...
const TVShowModal = ({ show, onClose }) => {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-2">
      <div className="bg-gradient-to-b from-[#1a1a1a] to-[#4c0519] text-white p-3 sm:p-8 rounded-lg flex flex-col sm:flex-row w-full max-w-3xl h-auto">
        <img
          src={show.image}
          alt={show.title}
          className="w-full sm:w-1/2 h-32 sm:h-auto object-cover rounded-md mb-2 sm:mb-0 sm:mr-3"
        />
        <div className="flex flex-col justify-between w-full sm:w-1/2 text-xs sm:text-sm">
          <h2 className="text-lg sm:text-2xl font-bold mb-1 sm:mb-2">{show.title}</h2>
          <p className="text-yellow-400 text-sm sm:text-lg mb-1 sm:mb-2">{show.rating}</p>
          <p className="text-sm sm:text-lg mt-1 sm:mt-2">{show.largeDescription}</p>
        </div>
        <button className="absolute top-2 right-2 text-white text-xl sm:text-2xl" onClick={onClose}>
          &times;
        </button>
      </div>
    </div>
